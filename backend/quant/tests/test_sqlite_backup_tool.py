...
class SQLiteBackupToolTests(unittest.TestCase):
    def test_backup_and_retention(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            script = Path(__file__).resolve().parents[1] / "tools" / "sqlite_backup.py"
            for _ in range(3):
            ...
            backups = sorted(backup_dir.glob("quant_api_*.db"))
