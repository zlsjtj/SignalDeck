...
BACKUP_DIR="logs/db_cold_backups"
PREFIX="quant_api_cold"
RETAIN="7"
LOG_FILE="logs/sqlite_cold_backup_cron.log"
MARKER="quant_sqlite_cold_backup_job"
DRY_RUN="false"

usage() {
  cat <<'EOF'
Install or update cron entry for sqlite_cold_backup.py.

Options:
  --python <path>          Python executable (default: /usr/bin/python3)
  --project-dir <path>     Project root (default: parent of this script)
  --schedule "<cron expr>" Cron schedule (default: 50 2 * * *)
...
ENTRY="${SCHEDULE} cd ${PROJECT_DIR} && ${PYTHON_BIN} tools/sqlite_cold_backup.py --db-path ${DB_PATH} --backup-dir ${BACKUP_DIR} --prefix ${PREFIX} --retain ${RETAIN} --verify >> ${LOG_FILE} 2>&1 # ${MARKER}"
